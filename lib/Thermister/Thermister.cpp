...
void Thermister::begin(uint8_t addr)
{
    ...
    {
        FANHUB_DEBUG_PRINTLN("Thermister not ready");
        return;
    }
    FANHUB_DEBUG_PRINTLN("Thermister ready");
...
float Thermister::readResistance(ThermisterChannel channel)
{
    ...
    if (adc_value == 0)
    {
        FANHUB_DEBUG_PRINTLN("ADC value is zero");
        return 0.0;
    }
    ...
    if (adc_value > MAX_ADC_VALUE)
    {
        FANHUB_DEBUG_PRINT("ADC value out of range: ");
        FANHUB_DEBUG_PRINTLN(adc_value);
        return 0.0;
    }
